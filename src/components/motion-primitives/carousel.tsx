  ...
  return (
    ...
    >
      <button
        ...
        className={cn(
          "pointer-events-auto flex size-10 items-center justify-center rounded-full border border-line bg-surface text-ink-2 shadow-sm transition-colors hover:border-accent hover:text-accent",
        ...
        {Array.from({ length: itemsCount }, (_, i) => (
          <button
            ...
            className={cn(
              ...
              index === i ? "w-6 bg-accent" : "w-2 bg-surface-2 hover:bg-ink-3",
              classNameButton,
            )}
          />
        ))}
...
function CarouselItem({ children, className }: CarouselItemProps) {
  return (
    <motion.div className={cn("w-full min-w-0 shrink-0 grow-0", className)}>{children}</motion.div>
  )
}
